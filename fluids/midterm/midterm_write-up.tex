...
\begin{homeworkProblem}
    ...
    Figure~\ref{fig:ftbs} demonstrates the result of running a downstream
    scheme. The numerical progression of $y(x,t)$ diverged. $\alpha$ is now $>
    2$ hence the amplitude will diverge from solution because the FTBS scheme is
    calculating the progression of $y$ from downstream instead of upstream.
...
\begin{homeworkProblem}
    ...
    needs more time steps to reach the same time as for the $\alpha = 0.5$
...
\begin{homeworkProblem}
    ...
    \noindent which allows us to solve for future time steps from an initial
    condition.\\
    ...
    \begin{figure}[!ht]
    \begin{center}
        \includegraphics[width=0.75\columnwidth]{q8.png} % Example image
    ...
    \noindent to get the solution before a single time-step we set $a \Delta
    t / \Delta x = \alpha$ and substitute the initial condition

    \begin{equation}
        y_k^{-1} = \sin[2 \pi k \Delta x] + \frac{\alpha}{2} 
        [\sin[2 \pi (k+1) \Delta x] - \sin[2 \pi k \Delta x]]
    \end{equation}

    \noindent which can be related as

    \begin{equation}
        y_k^{-1} = \sin[2 \pi k \Delta x] + \alpha
        [\sin[2 \pi \Delta x] \cos[2 \pi k \Delta x]]
    \end{equation}
    ...
    \begin{equation}
        y_k^{n-1} = A^{n-1} \sin[2 \pi \Delta x + (n-1) \phi]
    \end{equation}
    
    \noindent where

    \begin{equation}\label{eq:amp_ftfs}
        A = \sqrt{1+\alpha^2 \sin^2[2 \pi \Delta x]}
        \,\,\,\,\,\,\,\,\,\,\,\,
        \phi = {\rm atan2}[\alpha \sin[2 \pi \Delta x ], 1] 
    \end{equation}

    For any choice of $\alpha$, $A > 1$, thus previous time steps will have
    greater amplitudes than future time steps. Our solution will always be
    stable.}
