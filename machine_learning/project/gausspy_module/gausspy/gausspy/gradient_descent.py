...
def single_training_example(kwargs):
    ...
    status, result = AGD_decomposer.AGD(kwargs['vel'][j], kwargs['data'][j], 
                                        ...
                                        perform_final_fit=False, return_explicit_components=False,
...
def train(objective_function=objective_function, training_data=None, alpha1_initial = None, 
          alpha2_initial = None, iterations = 200, MAD = 0.20, eps = 0.25, learning_rate = 0.9, 
          p = .8, window_size = 10, iterations_for_convergence = 10, plot=False, phase=None, SNR2_thresh=0., 
    ...
    for i in range(iterations):
        ...
        print ''
        print 'iter {0}: F1={1}%, alpha=[{2}, {3}], p=[{4}, {5}]'.format(i, int(100 * np.exp(-gd.accuracy_trace[i])), np.round(gd.alpha1_trace[i],2), np.round(gd.alpha2_trace[i],2), np.round(momentum1,2), np.round(momentum2,2)),
