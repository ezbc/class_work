...
fit1 <- smooth.Pspline(x, y, spar=1, method=1)
...
postscript("fit1.ps", height=6, width=6, horizo=F)
name.pic <- paste("lambda =",as.character(fit1$spar))
plot(x,y,xlab="x",ylab="y",main=name.pic)
lines(fit1$x, fit1$y)
graphics.off()
...
###Try smaller lambda###
fit2 <- smooth.Pspline(x, y, spar=0.1, method=1)
...
### then create a ps file for LaTeX: ###
postscript("fit2.ps", height=6, width=6, horizo=F)
name.pic <- paste("lambda=",as.character(fit2$spar))
plot(x,y,xlab="x",ylab="y",main=name.pic)
lines(fit2$x, fit2$y)
graphics.off()
